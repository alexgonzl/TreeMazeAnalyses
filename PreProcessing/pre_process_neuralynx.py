...
def get_process_save_probe(task, save_format='bin', AmpPercentileThr=0.975, overwriteFlag=0):
    ...
    if not (sp / (fn + '.' + save_format)).exists() or overwriteFlag:
        ...
        try:
            ...
            print('Could not read first channel. Aborting.', sys.exc_info()[0], sys.exc_info()[1],
        ...
        info = {'RefChan': h1['RefChan'], 'fs': h1['fs'], 'AmpPercentileThr': AmpPercentileThr, 'nSamps': nSamps,
                'date_processed': date_str, 'subSessionID': ss, 'AD': [], 'InputRange': [], 'AmpRejThr': [],
                'date_created': [], 'sig_stats': [], 'fsig_stats': [], 'nBadAmpSamps': [], 'nAvgBadSamps': []}
